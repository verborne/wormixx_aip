...
class Minigun:
    def __init__(self, x, y):
        ...
        self.bullet_delay = 40  # Задержка между пулями (в миллисекундах)
        self.bullets_to_shoot = 15
        self.shooting = False
    ...
    while running:
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:  # Проверяем нажатие клавиши
                if event.key == pygame.K_SPACE and rifle.bullets_to_shoot == 15:  # Проверяем, что можно стрелять
            ...
            if event.type == pygame.KEYUP:  # Проверяем отпускание клавиши
                if event.key == pygame.K_SPACE:
                    ...
                    rifle.bullets_to_shoot = 15  # Сбросить количество пуль
